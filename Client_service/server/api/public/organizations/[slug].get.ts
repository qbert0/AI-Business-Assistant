...
export default defineEventHandler(async (event) => {
  ...
  return {
    ...
    suggestedQuestions: Array.isArray(organization.settings?.suggested_questions)
      ? organization.settings.suggested_questions.filter((item: unknown): item is string => typeof item === 'string')
      : [],
    documentTree: Array.isArray(organization.settings?.document_tree)
      ? organization.settings.document_tree
      : []
  }
})
