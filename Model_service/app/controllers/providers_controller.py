...
def list_providers(
    ...
    return RegistryService.from_dependencies(db, get_settings()).list_providers(name=name)
...
def create_provider(payload: models.ProviderCreate, db: Session = Depends(get_db)) -> models.ProviderRead:
    return RegistryService.from_dependencies(db, get_settings()).create_provider(payload)
...
def get_provider(provider_id: str, db: Session = Depends(get_db)) -> models.ProviderRead:
    return RegistryService.from_dependencies(db, get_settings()).get_provider(provider_id)
...
def update_provider(
    ...
    return RegistryService.from_dependencies(db, get_settings()).update_provider(provider_id, payload)
...
def delete_provider(provider_id: str, db: Session = Depends(get_db)) -> Response:
    RegistryService.from_dependencies(db, get_settings()).delete_provider(provider_id)
