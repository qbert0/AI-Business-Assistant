...
def update_my_settings(
    ...
    user = SettingsService(db).update_my_settings(payload, current_user)
...
def get_user_settings(user_id: str, db: Session = Depends(get_db)) -> models.UserRead:
    user = SettingsService(db).get_user_settings(user_id)
    return common_dto.to_user_model(user)
...
def update_user_settings(user_id: str, payload: models.UserSettingsUpdate, db: Session = Depends(get_db)) -> models.UserRead:
    user = SettingsService(db).update_user_settings(user_id, payload)
    return common_dto.to_user_model(user)
...
def get_organization_settings(
    ...
    settings = SettingsService(db).get_organization_settings(org_id, acting_user_id)
...
def update_organization_settings(
    ...
    settings = SettingsService(db).update_organization_settings(org_id, payload, acting_user_id)
    return settings_dto.to_organization_settings_model(settings)
